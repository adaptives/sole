...
public class Bootstrap extends Job {
	...
	private void createKVData() {
		KeyValueData kvData = 
				new KeyValueData("recently_pub_ss", 
	...
	private void createPages() {
		...
		String helpPageContent = 
		"<div style=\"font-size: 1.2em; line-height:130%; color: #A9B2C1;\">" + "\n" +
	...
	private void createUsers() {
		...
		SocialUser testUser = new SocialUser("testuser@somewhere.com", 
												"testuser");
		Role adminRole = Role.find("select r from Role r where r.name = ?", "admin").first();
		testUser.roles.add(adminRole);
		testUser.save();
		User uTestUser = new User("testuser@somewhere.com", "secret", "Test User", testUser);
		uTestUser.save();
		
		//anothertestuser
		SocialUser anotherTestUser = new SocialUser("anothertestuser@somewhere.com",
													"anothertestuser");
		anotherTestUser.save();
		User uAnotherTestUser = new User("anothertestuser@somewhere.com", "secret", "anothertestuser", anotherTestUser);
		uAnotherTestUser.save();
		
		//learner
		SocialUser learner = new SocialUser("learner@somewhere.com", 
				                            "learner");
		learner.save();
		User uLearner = new User("learner@somewhere.com", "secret", "learner", learner);
		uLearner.save();
	}
